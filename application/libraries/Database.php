...
class DatabaseLibrary {
	...
	{
		if ( ! isset(DatabaseLibrary::$instances[$name]))
		{
			// Create a new instance
			DatabaseLibrary::$instances[$name] = new DatabaseLibrary($config === NULL ? $name : $config);
		}

		return DatabaseLibrary::$instances[$name];
	}
	...
	{
		return array_search($db, DatabaseLibrary::$instances, TRUE);
	}
	...
	{		
		if (empty($config))
		{
			...
			$config = Config::Instance()->database['default'];
		}
		...
		// Set driver name
		$driver = 'Database_'.inflector::camelize($this->config['connection']['type']).'_Driver';
	...
	/**
	 * Resets the selected query builder
	 *
	 * @return  void
	 */
