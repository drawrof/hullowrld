...
class ORMLibrary {
	...
	public function __construct($id = NULL)
	{
		...
		$this->object_plural = inflector::pluralize($this->object_name);
	 ...
	 * @return  void
	 */
	public function __initialize_db()
	{
	...
	public function __get($column)
	{
		...
		elseif (isset($this->has_many[$column]))
		{
			...
			$through = ORMLibrary::Factory(inflector::singularize($this->has_many[$column]));

			// Load the "end" model
			$model = ORMLibrary::Factory(inflector::singularize($column));
		...
		elseif (in_array($column, $this->has_many))
		{
			...
			return $this->related[$column] = ORMLibrary::Factory(inflector::singularize($column))
	...
	public function validate(Validation $array, $save = FALSE)
	{
		if ( ! $array->submitted())
		{
			...
			foreach ($safe_array as $key => $value)
			{
				...
				if (is_object($value) AND $value instanceof ORMIteratorLibrary)
				{
	...
	public function reload_columns($force = FALSE)
	{
		if ($force === TRUE OR empty($this->table_columns))
		{
			...
			{
				// Load table columns
				self::$column_cache[$this->object_name] = $this->table_columns = $this->list_fields($this->table_name, TRUE);
			}
		}
	 ...
	 * @return  boolean
	 */
	public function has(ORMLibrary $model, $any = FALSE)
	{
	 ...
	 * @return  boolean
	 */
	public function add(ORMLibrary $model)
	{
		if ($this->has($model))
			return TRUE;
	 ...
	 * @return  boolean
	 */
	public function remove(ORMLibrary $model)
	{
		if ( ! $this->has($model))
			return FALSE;
	...
	protected function related_object($object)
	{
		...
		{
			$object = ORMLibrary::Factory($this->has_one[$object]);
		}
		elseif (isset($this->belongs_to[$object]))
		{
			$object = ORMLibrary::Factory($this->belongs_to[$object]);
		}
		elseif (in_array($object, $this->has_one) OR in_array($object, $this->belongs_to))
		{
			$object = ORMLibrary::Factory($object);
		}
		else
		{
			return FALSE;
	 ...
	 * @return  ORMIteratorLibrary  for multiple rows
	 */
	protected function load_result($array = FALSE)
	{
		...
		if ($array === TRUE)
		{
			...
			return new ORMIteratorLibrary($this, $result);
		}
	 ...
	 * @return  array
	 */
	protected function load_relations($table, ORMLibrary $model)
	{
