...
class Database_Mysql_Driver extends Database_Driver {
	...
	public function query($sql)
	{
		...
		if ($this->db_config['cache'] AND ! preg_match('#\b(?:INSERT|UPDATE|REPLACE|SET)\b#i', $sql))
		{
			...
			if ( ! isset(self::$query_cache[$hash]))
			{
				...
				self::$query_cache[$hash] = new MysqlResult(mysql_query($sql, $this->link), $this->link, $this->db_config['object'], $sql);
			}
		...
		return new MysqlResult(mysql_query($sql, $this->link), $this->link, $this->db_config['object'], $sql);
	}
	...
	public function list_tables(DatabaseLibrary $db)
	{
		static $tables;
...
class MysqlResult extends DatabaseResult {
	...
	public function result($object = TRUE, $type = MYSQL_ASSOC)
	{
		...
		{
			$this->return_type = (is_string($type) AND App::autoload($type)) ? $type : 'stdClass';
		}
		else
		{
	...
	public function result_array($object = NULL, $type = MYSQL_ASSOC)
	{
		...
		elseif (is_bool($object))
		{
			if ($object === TRUE)
			{
				...
				$type = (is_string($type) AND App::autoload($type)) ? $type : 'stdClass';
			}
			else
			{
		...
		else
		{
			...
			{
				$type = (is_string($this->return_type) AND App::autoload($this->return_type)) ? $this->return_type : 'stdClass';
			}
		}
