...
class App {
	...
	{				
		...
		// Helper
			...
			$type = "Helper";
			$path = self::check_path($fullname.'_helper',APP_HELPER_DIR);
			...
			// Then System Helpers
				...
				$path = self::check_path($fullname.'_helper',HELPER_DIR);	
			}
		...
		// Drivers
			...
			// Split apart at uppercase, pop off the last segment (== 'Driver')
			$segments = explode(' ',preg_replace('/(\B[A-Z])(?=[a-z])|(?<=[a-z])([A-Z])/sm', ' $1$2', $fullname));
			array_pop($segments);
						
			if (count($segments) === 1) {
				$path = self::check_path($segments[0],DRIVER_DIR);
			} else {
				$path = self::check_path($segments[0].'/'.$segments[1],DRIVER_DIR);
			}
	...
	static function check_path($file, $path, $ext = 'php') 
	{
		...
		if (strrpos($file,'.') === false && $ext != false){ 
			$file = $file.".".$ext; 
		}
