...
class ImageLibrary {
    ...
    public function __construct($fileName) {
    	...
    	if(!function_exists("gd_info")) {
        	throw new Error('missing_gd_extension');
        }
        ...
        if(!file_exists($this->fileName)) {
            $this->errmsg = 'missing_file';
            $this->error = true;
        }
        //check to see if file is readable
        elseif(!is_readable($this->fileName)) {
            $this->errmsg = 'unreadable_file';
            $this->error = true;
        }
        ...
        if($this->error == false) {
			...
			if ($this->format !== IMAGETYPE_GIF && $this->format !== IMAGETYPE_JPEG && $this->format !== IMAGETYPE_PNG){
				$this->errmsg = 'unknown_format';
                $this->error = true;
			} 
        }
	...
	{
		return new ImageLibrary($file);
	}
	
	private function getImageFormat()
	{
		if ( !function_exists( 'exif_imagetype'))  {
		        if ((list($width, $height, $type, $attr) = getimagesize($this->fileName)) !== false) {
		            return $type;
		        }
		    return false;
		} else {
